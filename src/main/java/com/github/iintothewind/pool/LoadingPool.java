...
import com.github.iintothewind.util.Preconditions;
...
public class LoadingPool<T> implements Pool<T> {
  ...
  private final static Logger log = LoggerFactory.getLogger(LoadingPool.class);
  ...
  private LoadingPool(final int minSize, final int maxSize, final int parallelism, final Duration defaultBorrowTimeout, final Supplier<T> supplier) {
    Preconditions.checkArg(minSize > 0, "minSize is required bigger than 0");
    Preconditions.checkArg(maxSize < 99, "maxSize is required smaller than 99");
    Preconditions.checkArg(minSize < maxSize, "maxSize is required smaller than 99");
    Preconditions.checkArg(parallelism > 0, "parallelism is required bigger than 0");
